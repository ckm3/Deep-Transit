...
from megengine.functional.tensor import transpose
...
class Upsample(nn.Module):
    ...
    def forward(self, inputs):
        return F.vision.interpolate(inputs,scale_factor=self.scale)
...
class ScalePrediction(nn.Module):
    ...
    def forward(self, x):
        return (
            F.transpose( self.pred(x)
                .reshape(x.shape[0], num_anchors // 3, 5, x.shape[2], x.shape[3]), (0, 1, 3, 4, 2))
        )
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            if isinstance(layer, ResidualBlock) and layer.num_repeats == 8:
                ...
                x = F.concat([x, route_connections[-1]], axis=1)
...
if __name__ == "__main__":
    ...
    x = random.randn(2, 1, IMAGE_SIZE, IMAGE_SIZE).astype('float32')
